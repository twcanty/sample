...
do_open(const char *filename, int oflags)
{
        ...
        vnode_t *base = NULL;
        vnode_t *res_vnode;
        int ret = open_namev(filename, oflags, &res_vnode, base);
        if (ret < 0) {
            fput(file);
            curproc->p_files[fd] = NULL;
            return ret;//THIS HANDLES -ENOENT
        }
        ...
        //need to have appropriate devices
        if ((S_ISCHR(res_vnode->vn_mode)&& (bytedev_lookup(res_vnode->vn_devid)==NULL)) 
            ||(S_ISBLK(res_vnode->vn_mode)&&(blockdev_lookup(res_vnode->vn_devid)==NULL))) {
            fput(file);
            curproc->p_files[fd] = NULL;
            vput(res_vnode);
            return -ENXIO;
